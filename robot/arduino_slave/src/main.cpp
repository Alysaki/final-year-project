...
// --- Control Data (From Pi -> ESP32) ---
float shared_targetSpeedL = 0;
float shared_targetSpeedR = 0;

// --- Telemetry Data (From ESP32 -> Pi) ---
long shared_encLeftCount = 0;
long shared_encRightCount = 0;
float shared_yaw = 0.0;
int16_t shared_ax=0, shared_ay=0, shared_az=0, shared_gx=0, shared_gy=0, shared_gz=0;
...
class MyCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
      ...
      if (jsonStr.length() > 0) {
        ...
        logToPi("Action: " + action + " | Token: " + token + " | Expected: " + localToken);
...
void IRAM_ATTR isrLeftEncoder() {
  if (dirLeftForward) encLeftCount++;
  else encLeftCount--;
}

void IRAM_ATTR isrRightEncoder() {
  if (dirRightForward) encRightCount++;
  else encRightCount--;
}

// readMPU6050 has been deleted to use Adafruit library

void runPID(long deltaL, long deltaR, float targetL, float targetR, bool obstacleDetected) {
  if (targetL == 0 && targetR == 0) {
    digitalWrite(IN1, LOW); digitalWrite(IN2, LOW);
    digitalWrite(IN3, LOW); digitalWrite(IN4, LOW);
    ledcWrite(pwmChannelA, 0); ledcWrite(pwmChannelB, 0);
    errorL_integral = 0; errorR_integral = 0;
    last_errorL = 0; last_errorR = 0;
    return;
  }
  ...
  if (dirLeftForward) { digitalWrite(IN1, HIGH); digitalWrite(IN2, LOW); } 
  ...
  if (dirRightForward) { digitalWrite(IN3, HIGH); digitalWrite(IN4, LOW); } 
...
void handlePiCommand(String cmd) {
  ...
  if (cmd.startsWith("SPEED:")) {
    ...
    if (firstColon != -1 && secondColon != -1) {
      if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
        shared_targetSpeedL = cmd.substring(firstColon + 1, secondColon).toFloat();
    ...
    if (isBoxEmpty) Serial.println("BOX:EMPTY");
    else Serial.println("BOX:FULL");
  } else if (cmd == "SYS:CHECK") {
    if (sysReady) Serial.println("SYS:READY");
    else Serial.println("SYS:ERROR:" + sysErrorMsg);
  }
}
  ...
  for (;;) {
    ...
    if (sysReady && (currentMillis - lastTelemetryTime >= 50)) {
      ...
      if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
        ...
        y = shared_yaw;
        a_x = shared_ax; a_y = shared_ay; a_z = shared_az;
        g_x = shared_gx; g_y = shared_gy; g_z = shared_gz;
        gz_d = shared_gz_deg;
      ...
      char txBuf[128];
      snprintf(txBuf, sizeof(txBuf), "ODO:%ld:%ld:%.2f:%d:%d:%d:%d:%d:%d:%.2f:%.1f:%d",
               encL, encR, y,
               a_x, a_y, a_z,
               g_x, g_y, g_z,
               gz_d, batt, t_dist);
    ...
    vTaskDelay(10 / portTICK_PERIOD_MS); // Yield CPU Core 0 to Wi-Fi/BLE
  }
}
...
void TaskControl(void *pvParameters) {
  ...
  for (;;) {
    ...
    // 0. If hardware is faulty, stop the algorithm
    if (!sysReady) {
    ...
    if (tofFrontReady) {
      ...
      // When the object is suddenly lifted, the sensor may experience phase failure or signal failure
      // resulting in RangeStatus != 4 but RangeMilliMeter is 0 or garbage.
      if (measure.RangeStatus != 4 && measure.RangeMilliMeter > 20 && measure.RangeMilliMeter < 8000) {
        ...
        // Out of range (cleared weather) or noise.
        // If there was no obstacle before, we assume the distance is infinite (9999)
        // If there WAS an obstacle before, we KEEP THE STATE (because the obstacle may be right in front of the sensor < 2cm causing blindness)
    ...
    if (mpuReady) {
      ...
      if (Wire.available() == 14) {
        ...
        if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
          ...
          shared_ax = ax; shared_ay = ay; shared_az = az;
          shared_gx = gx; shared_gy = gy; shared_gz = gz;
          xSemaphoreGive(dataMutex);
        }
      }
    }
    ...
    if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
      if (currentServoAngle != targetServoAngle && (currentMillis - lastServoStepTime >= 5)) {
      ...
      // Box Logic: Close the lid after 8s if the goods state changes (sender puts goods in, or receiver takes goods out)
      if (doorState == DOOR_OPEN) {
        ...
        bool currentBoxState = (digitalRead(LIMIT_SWITCH_PIN) == LOW);
        if (currentBoxState == initialBoxState) {
          // If the state returns to the same as before (e.g., put in and then take out again), cancel the countdown
          doorState = DOOR_OPEN;
        } else if (currentMillis - boxFullTime >= 8000) {
          // Kept the new state for 8 seconds
          targetServoAngle = 100; // Close the lid
    ...
    if (currentMillis - lastPidTime >= 50) {
      ...
      // read target speed from shared variables (mutex)
      float tL = 0, tR = 0;
      if (xSemaphoreTake(dataMutex, portMAX_DELAY)) {
        ...
        // Variables ax,ay,az,gx,gy,gz, and yaw have been continuously updated in section (2. MPU9250)
        // so they don't need to be assigned again here.
    ...
    // Core 1 Delay (Run at max 200Hz = 5ms loop to ensure ToF/IMU are read fast)
...
void setup() {
  ...
  Wire.setTimeOut(150); // Increased to 150ms for ToF VL53L0X to respond (20ms is too fast)

  // 1. Initialize MPU6050 and Compass (0x2C)
  Wire.beginTransmission(MPU6050_ADDR);
  ...
  filter.begin(200.0); // TaskControl loop frequency ~200Hz

  // 2. Initialize VL53L0X next
  if (loxFront.begin()) {
    tofFrontReady = true;
  } else {
    tofFrontReady = false; 
  ...
  // Temporarily disable ToF error to allow the car to run without ToF
  // if (!tofFrontReady) sysErrorMsg += "ToF_Front ";
  
  // if (!mpuReady) sysErrorMsg += "MPU6050 ";
  if (sysErrorMsg == "") sysReady = true;

  /*
  if (mpuReady) {
    Serial.println("Calibrating MPU9250 (Keep the car still for 3 seconds)...");
    mpu.calibrateAccelGyro();
    Serial.println("Calibration Done!");
  }
  */

  pinMode(ENC_L_PIN, INPUT_PULLUP); pinMode(ENC_R_PIN, INPUT_PULLUP);
  ...
  BLECharacteristic *pRxCharacteristic = pService->createCharacteristic(
      CHARACTERISTIC_UUID_RX, BLECharacteristic::PROPERTY_WRITE | BLECharacteristic::PROPERTY_NOTIFY
  );
...
void loop() {
  // Left empty because FreeRTOS has taken over control
  vTaskDelete(NULL);
}
