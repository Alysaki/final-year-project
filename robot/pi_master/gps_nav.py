...
class GpsNavigator:
    ...
    def fetch_route(self, dest_lat, dest_lon):
        ...
        # Use public OSRM server since MapTiler route API returns 404
        url = f"https://router.project-osrm.org/route/v1/foot/{self.current_lon},{self.current_lat};{dest_lon},{dest_lat}"
        params = {
            'steps': 'true'
        }
    ...
    def get_navigation_commands(self, telemetry):
        ...
        lookahead_dist = 1.0  # 1 meter lookahead
        target_pt = None
        
        # Search forward on path for a point at lookahead distance
        ...
        # If the angle of deviation is too large (> 45 degrees / ~0.8 rad), adjust the zoom level (Spin in Place) to reorient.
        if abs(heading_error) > 0.8:
            turn_speed = 15.0  # Tốc độ quay tại chỗ (ticks/50ms)
            if heading_error > 0:
                # Turn left: left wheel rotates backward, right wheel rotates forward
                return -turn_speed, turn_speed, False
            else:
                # Turn right: left wheel rotates forward, right wheel rotates backward
                return turn_speed, -turn_speed, False
        ...
        w = v_base * kappa  # Angular velocity target
