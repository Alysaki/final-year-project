...
public class CreateTaskActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Apply padding to root so the keyboard pushes the entire CoordinatorLayout up
        EdgeToEdgeHelper.applyNavigationBarPadding(binding.getRoot());
        ...
        try {
            ...
            mapStyle.addLayer(new CircleLayer(LAYER_MARKER_A, SOURCE_MARKER_A)
                    .withProperties(
                            ...
                            PropertyFactory.circleColor("#1B8A4B"),   // green
            ...
            mapStyle.addLayer(new CircleLayer(LAYER_MARKER_B, SOURCE_MARKER_B)
                    .withProperties(
                            PropertyFactory.circleRadius(11f),
                            PropertyFactory.circleColor("#D32F2F"),   // red
                            PropertyFactory.circleStrokeWidth(3f),
                            PropertyFactory.circleStrokeColor("#FFFFFF")));
            ...
            binding.mapView.post(() -> {
                if (binding == null || isFinishing() || isDestroyed()) return;
                try {
                    cachedMap.moveCamera(CameraUpdateFactory.newLatLngBounds(bounds, 120));
                } catch (Exception e) {
                    android.util.Log.w("CreateTaskMap", "bounds zoom failed, centering");
