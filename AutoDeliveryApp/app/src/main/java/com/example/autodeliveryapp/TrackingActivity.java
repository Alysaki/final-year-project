...
public class TrackingActivity extends AppCompatActivity {
        ...
        try {
            ...
            mapStyle.addLayer(new CircleLayer(LAYER_MARKER_A, SOURCE_MARKER_A)
                    .withProperties(
                            ...
                            PropertyFactory.circleColor("#1B8A4B"),
            ...
            if (robotLatLng != null) {
                ...
                if (mapStyle.getImage("agv-icon") == null) {
                    android.graphics.Bitmap agvIcon = getBitmapFromVectorDrawable(this, R.drawable.ic_agv_top);
                    if (agvIcon != null) {
    ...
    private void setupFirebaseListener(String currentUid, String senderUid) {
        ...
        taskListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (fbRouteGeoJson != null && !fbRouteGeoJson.isEmpty()) {
    ...
    private void updateBleButtonVisibility() {
        ...
        if (isSender) {
            ...
            if (TaskStatus.ARRIVED_DROPOFF.equals(currentTaskStatus) || TaskStatus.WAITING_RECEIVER_UNLOCK.equals(currentTaskStatus)) {
                ...
                if (!isReceiverTokenVerified) {
                    binding.btnBleAction.setText("Tap to scan and verify AGV");
                    binding.btnBleAction.setEnabled(true);
                    if (binding.tvBleSenderName != null) {
                        binding.tvBleSenderName.setVisibility(View.GONE);
                    }
                } else {
                    if (binding.tvBleSenderName != null) {
                    ...
                    binding.btnBleAction.setText("Unlock compartment");
    ...
    private void handleBleAction() {
        ...
        if (isSender) {
            ...
            action = isReceiverTokenVerified ? BleConstants.ACTION_OPEN_SLOT : BleConstants.ACTION_VERIFY_TOKEN;
        }
        ...
        bleManager.executeBleAction(currentRobotId, action, currentOrderId, currentBleToken, new BleActionCallback() {
            ...
            public void onSuccess() {
                runOnUiThread(() -> {
                    if (!isSender && !isReceiverTokenVerified) {
                        isReceiverTokenVerified = true;
                        Toast.makeText(TrackingActivity.this, "AGV verified successfully!", Toast.LENGTH_SHORT).show();
                        updateBleButtonVisibility();
                    } else {
                        Toast.makeText(TrackingActivity.this, "Command sent successfully!", Toast.LENGTH_SHORT).show();
