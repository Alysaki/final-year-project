...
secrets {
    ignoreList.add("MQTT_PORT")
    ignoreList.add("MQTT_USE_SSL")
}

val mqttPortValue = mqttPort.trim().toIntOrNull()
    ?.takeIf { it in 1..65535 }
    ?: throw GradleException(
        "MQTT_PORT must be an integer from 1 to 65535; current value: '$mqttPort'"
    )

val mqttUseSslValue = when (mqttUseSsl.trim().lowercase()) {
    "true" -> true
    "false" -> false
    else -> throw GradleException(
        "MQTT_USE_SSL must be true or false; current value: '$mqttUseSsl'"
    )
}
